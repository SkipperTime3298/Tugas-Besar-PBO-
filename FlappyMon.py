...
while isGameRun :    
    ...
    for i in range (2):
        ground = Ground(GROUND_WIDTH * i, groundMenuGame)
    ...
    while(gameState == "menuGame") :
        ...
        if is_off_screen(ground_group.sprites()[0]):
            ...
            new_ground = Ground(GROUND_WIDTH - 20, groundMenuGame)
            ...
            if(delay_menu > 0) :
                ...
                if(select_mouse_POS[0] in range(btn_chiko_Rect.left, btn_chiko_Rect.right) and 
                   select_mouse_POS[1] in range(btn_chiko_Rect.top, btn_chiko_Rect.bottom)) :
                    ...
                    for i in range (2):
                        ground = Ground(GROUND_WIDTH * i,charSelect)
                        ground_group.add(ground)
    ...
    while(gameState == "playGame") :
        ...
        if is_off_screen(ground_group.sprites()[0]):
            ground_group.remove(ground_group.sprites()[0])
            new_ground = Ground(GROUND_WIDTH - 20, charSelect)
            ground_group.add(new_ground)
        ...
        if len(pipe_group)>0:
            charSelect.get_score(pipe_group)
